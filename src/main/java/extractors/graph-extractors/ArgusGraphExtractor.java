	 	...
	 	for (int i = start; i < records.size(); i++) {
			...
			if (!record.get(SOURCE_ADDRESS).isEmpty()) {
				...
				if (vertNames.containsKey(srcIp)) {
					...
					JSONObject srcIpJson = GraphUtils.setIpJson(srcIp, srcIpID, source, "Argus");
			...
			/* source port */
				...
				if (vertNames.containsKey(srcPort)) {
					...
					JSONObject srcPortJson = GraphUtils.setPortJson(srcPort, source, "Argus");
			...
			if (!record.get(DESTINATION_ADDRESS).isEmpty()) {
				...
				if (vertNames.containsKey(dstIp)) {
					...
					JSONObject dstIpJson = GraphUtils.setIpJson(dstIp, dstIpID, source, "Argus");
			...
			if (!record.get(DESTINATION_PORT).isEmpty()) {
				...
				if (vertNames.containsKey(dstPort)) {
					...
					JSONObject dstPortJson = GraphUtils.setPortJson(dstPort, source, "Argus");
			...
			if (srcIpID != null && srcPortID != null) {
				...
				if (vertNames.containsKey(address)) {
					...
					JSONObject srcAddressJson = GraphUtils.setAddressJson(srcIp, srcIpID, srcPort, srcPortID, source, "Argus");
			...
			if (dstIpID != null && dstPortID != null) {
				...
				if (vertNames.containsKey(address)) {
					...
					JSONObject dstAddressJson = GraphUtils.setAddressJson(dstIp, dstIpID, dstPort, dstPortID, source, "Argus");
			...
			if (srcAddressID != null && dstAddressID != null) {
				...
				if (vertNames.containsKey(flow)) {
					...
					JSONObject flowJson = GraphUtils.setFlowJson(flowID, srcIp, srcPort, srcAddressID, dstIp, dstPort, dstAddressID, protocol, source, "Argus", headersSet, record);
