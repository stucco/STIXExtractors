		...
		try	{
			...
			Observables observables = initObservables();
		 	...
		 	for (int i = start; i < records.size(); i++)	{
				...
				System.out.println("after softname");
				//source IP
				...
				//source address, source address -> source IP, source address -> source Port
				...
				//destination address, destination address -> destination IP, destination address -> destination Port
				...
				if (!record.get(DEST_IP).isEmpty() && !record.get(DEST_PORT).isEmpty() &&
					...
					flowObservable = new Observable()
						.withId(new QName("gov.ornl.stucco", "flow-" + UUID.randomUUID().toString(), "stucco"))
						.withTitle("Flow")
						.withObservableSources(getMeasureSourceType("Hone"))
						.withObject(new ObjectType()
							.withId(new QName("gov.ornl.stucco", "flow-" + record.get(SOURCE_IP) + "_" +record.get(SOURCE_PORT) + "-" + 
								record.get(DEST_IP) + "_" + record.get(DEST_PORT), "stucco"))
				...
				if (hostnameObservable != null)	{
					...
					if (softwareObservable != null)	
						relatedObjects			
							.withRelatedObjects(new RelatedObjectType() //list
								.withIdref(softwareObservable.getId())
								.withRelationship(new ControlledVocabularyStringType()
									.withValue("host runs software"))
								.withDescription(new StructuredTextType()
									.withValue(HOSTNAME + " runs " + record.get(PATH)))
								.withDiscoveryMethod(getMeasureSourceType("Hone")));

					//host -> source address relation
					if (srcAddressObservable != null)
						relatedObjects			
							.withRelatedObjects(new RelatedObjectType() //list
								.withIdref(srcAddressObservable.getId())
								.withRelationship(new ControlledVocabularyStringType()
									.withValue("host uses address"))
								.withDescription(new StructuredTextType()
									.withValue(HOSTNAME + " uses address " + record.get(SOURCE_IP) + ", port " + record.get(SOURCE_PORT)))
								.withDiscoveryMethod(getMeasureSourceType("Hone")));
				...
				if (softwareObservable != null)	{
					...
					if (flowObservable != null)
						relatedObjects
							.withRelatedObjects(new RelatedObjectType() //list
								.withIdref(flowObservable.getId())
								.withRelationship(new ControlledVocabularyStringType()
									.withValue("software has flow"))
								.withDescription(new StructuredTextType()
									.withValue(record.get(PATH) + " has flow " + 
										record.get(SOURCE_IP) + ", port " + record.get(SOURCE_PORT) +" to " + 
										record.get(DEST_IP) + ", port " + record.get(DEST_PORT)))
								.withDiscoveryMethod(getMeasureSourceType("Hone")));
					...
					if (accountObservable != null)
						relatedObjects
							.withRelatedObjects(new RelatedObjectType() //list
								.withIdref(accountObservable.getId())
								.withRelationship(new ControlledVocabularyStringType()
									.withValue("software runs as account"))
								.withDescription(new StructuredTextType()
									.withValue(record.get(PATH) + " runs as uid " + record.get(UID) + " on host " + HOSTNAME))
								.withDiscoveryMethod(getMeasureSourceType("Hone")));
